...
#pragma unroll
	for (int tile = 0; tile < numTiles; tile++) {
		...
		As[tx * REGTILE + 0][ty * REGTILE + 0] = rowA0.x;  // colA + 0
		As[tx * REGTILE + 1][ty * REGTILE + 0] = rowA0.y;  // colA + 1
		float2 rowA1 = *reinterpret_cast<const float2*>(&A[(rowA + 1) * K + colA]);
		As[tx * REGTILE + 0][ty * REGTILE + 1] = rowA1.x;  // colA + 0
		As[tx * REGTILE + 1][ty * REGTILE + 1] = rowA1.y;  // colA + 1
...
#pragma unroll
		for (int k = 0; k < TILESIZE; k++) {
			...
			float a0 = As[k][ty * REGTILE + 0];
			float a1 = As[k][ty * REGTILE + 1];
